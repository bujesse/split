...
func ExpenseForm(expense *models.Expense, categories []models.Category, currencies []models.Currency, users []models.User) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		_, templ_7745c5c3_Err = templ_7745c5c3_Buffer.WriteString("</div><div class=\"mb-4\"><textarea x-model=\"Notes\" id=\"notes\" name=\"notes\" class=\"textarea textarea-bordered w-full\" placeholder=\"Notes\"></textarea></div><div x-ref=\"recurrenceForm\" x-show=\"showRecurrenceForm\" x-transition x-cloak><h4 class=\"mb-6 text-2xl font-bold\">Schedule Expense</h4><input type=\"hidden\" x-model=\"showRecurrenceForm\" name=\"IsScheduled\"><div class=\"mb-4 flex space-x-4\"><div class=\"w-1/2\"><label class=\"form-control w-full max-w-xs\"><div class=\"label\"><span class=\"label-text\">Recurrence Type</span></div><select x-model=\"RecurrenceType\" name=\"RecurrenceType\" id=\"recurrenceType\" class=\"select select-bordered grow\" required><option value=\"monthly\">Monthly</option> <option value=\"daily\">Daily</option> <option value=\"weekly\">Weekly</option> <option value=\"yearly\">Yearly</option></select></label></div><div class=\"w-1/2\"><label class=\"form-control w-full max-w-xs\"><div class=\"label\"><span class=\"label-text\">Recurrence Interval</span></div><input type=\"number\" x-model=\"RecurrenceInterval\" name=\"RecurrenceInterval\" id=\"recurrenceInterval\" class=\"input input-bordered grow\" required placeholder=\"e.g. 2 weeks\"></label></div></div><div class=\"mb-4\"><label class=\"input input-bordered flex items-center gap-2\">Start Date <input x-model=\"StartDate\" type=\"date\" name=\"StartDate\" id=\"startDate\" class=\"grow\" required></label></div><div class=\"mb-4\"><label class=\"input input-bordered flex items-center gap-2\">End Date <input x-model=\"EndDate\" type=\"date\" name=\"EndDate\" id=\"endDate\" class=\"grow\"></label></div></div><div class=\"flex items-center justify-between\"><div>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		if expense != nil {
			...
			_, templ_7745c5c3_Err = templ_7745c5c3_Buffer.WriteString("<button class=\"btn btn-secondary w-full \" type=\"button\" @click=\"showRecurrenceForm = !showRecurrenceForm; $nextTick(() =&gt; { $refs.recurrenceForm.scrollIntoView({ behavior: &#39;smooth&#39; }); })\"><span x-text=\"showRecurrenceForm ? &#39;Hide Scheduler&#39; : &#39;Schedule&#39;\"></span></button>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
		}
